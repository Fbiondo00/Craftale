...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children, initialUser }) => {
  ...
  const convertSupabaseUser = (supabaseUser: SupabaseUser | null): User | null => {
    ...
    return {
      ...
      avatar:
        supabaseUser.user_metadata?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${supabaseUser.email}`,
    ...
    async (email: string, password: string, name: string) => {
      ...
      try {
        ...
        if (!result.success) {
          ...
          else if (result.error?.includes("Invalid email") || result.error?.includes("valid email")) {
