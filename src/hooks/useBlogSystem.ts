...
import { mockBlogPosts, defaultFilterState } from '@/data/mockBlogData';
...
export default function useBlogSystem() {
  ...
  const filteredPosts = useMemo(() => {
    ...
    if (filters.industries.length > 0) {
      filtered = filtered.filter(post => 
        post.industry.some(industry => filters.industries.includes(industry))
      );
    }

    if (filters.technologies.length > 0) {
      filtered = filtered.filter(post => 
        post.technologies.some(tech => filters.technologies.includes(tech))
      );
    }

    if (filters.serviceTypes.length > 0) {
      filtered = filtered.filter(post => 
        post.serviceTypes.some(service => filters.serviceTypes.includes(service))
      );
    }
    ...
    if (filters.projectSizes.length > 0) {
      filtered = filtered.filter(post => 
        filters.projectSizes.includes(post.projectSize)
      );
    }
      ...
      switch (sortBy) {
        ...
        case 'client-success':
          ...
          const aResults = a.clientResults?.salesIncrease ? 
            parseInt(a.clientResults.salesIncrease.replace(/\D/g, '')) : 0;
          const bResults = b.clientResults?.salesIncrease ? 
            parseInt(b.clientResults.salesIncrease.replace(/\D/g, '')) : 0;
