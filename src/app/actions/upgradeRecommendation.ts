...
const CheckUpgradeSchema = z.object({
  ...
  selected_services: z.array(z.union([z.string(), z.number()])).transform(
    arr => arr.map(val => Number(val))
  ).optional().default([])
})

// Check for upgrade recommendations
export async function checkUpgradeAction(
  ...
  try {
    ...
    const nextLevelCode = currentLevel.level_code === 'A' ? 'B' : 
                         currentLevel.level_code === 'B' ? 'C' : null
