...
export type ActionState = { success: true; data: { sessionId: string } } | { success: false; message: string };
...
export async function trackPricingJourneyAction(
  ...
  try {
    ...
    const { error } = await supabase.from("pricing_journey_events" as any).insert(insertData);
...
export async function trackPersonaMatcherAction(
  ...
  try {
    ...
    const { error } = await supabase.from("persona_matcher_responses").insert(insertData);
...
export async function trackServiceToggle(serviceId: string | number, selected: boolean) {
  return trackEvent(selected ? "service_added" : "service_removed", { service_id: serviceId });
}
