...
const proTierData: TierData = {
  ...
  name: 'Pro',
  description:
    'Esperienza interattiva moderna per nuovi imprenditori e specialisti regionali avanzati',
      ...
      features: [
        ...
        },
        {
          ...
          premium: {
            ...
            longDescription:
              'Menu interattivo con descrizioni dettagliate, ingredienti e allergeni',
          },
        },
      ],
    },
      ...
      features: [
        ...
        },
        {
          ...
          premium: {
            ...
            longDescription:
              'Gallery professionale con categorie tematiche (ambiente, piatti, eventi)',
          },
        },
        {
          ...
          base:
            selectedTier === 'starter'
              ...
              : {
                  ...
                  longDescription:
                    'Sistema di prenotazioni integrato con calendario e conferme automatiche',
                },
          standard:
            selectedTier === 'starter'
              ? false
              : {
                  ...
                  longDescription:
                    'Prenotazioni multi-slot con eventi speciali e gestione depositi',
                },
          premium:
            selectedTier === 'starter'
              ? {
          ...
          base:
            selectedTier === 'ecommerce'
              ? {
                  ...
                  longDescription:
                    'E-commerce completo per prodotti ristorante con payment gateway',
                }
              : false,
          standard:
            selectedTier === 'ecommerce'
...
export const optionalServicesData: OptionalService[] = [
  ...
  },
  {
    ...
    description:
      '40 foto + video breve 1 min. Giornata completa, video presentazione, footage extra',
    category: 'photography',
    price: 550,
    compatibleTiers: ['starter', 'pro', 'ecommerce'],
...
export const getLevelData = (tier: TierType, level: LevelType): LevelData =>
  tierData[tier].levels[level];

export const getTierPrice = (tier: TierType, level: LevelType): number =>
  tierData[tier].levels[level].price;
...
export const getOptionalServicesByCategory = (
  category: OptionalService['category']
): OptionalService[] => optionalServicesData.filter((service) => service.category === category);
...
export const getIncludedFeaturesForLevel = (tier: TierType, level: LevelType): string[] => {
  ...
  if (levelData.features.pages.some((page) => page.includes('Blog')))
    includedFeatures.push('blog_setup');
  if (levelData.features.pages.some((page) => page.includes('Eventi')))
    includedFeatures.push('events_page');
  ...
  // Check integrations
  if (
    levelData.features.integrations.some((integration) =>
      integration.includes('Google My Business')
    )
  )
  ...
  if (
    levelData.features.marketing.some((marketing) => marketing.includes('Social media management'))
  )
  ...
  if (
    levelData.support &&
    levelData.support.duration &&
    levelData.support.duration !== 'Non incluso'
  )
    includedFeatures.push('extended_support');
  if (levelData.training && levelData.training.sessions > 2)
    includedFeatures.push('training_sessions');
