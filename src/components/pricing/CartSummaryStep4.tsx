...
import { ArrowLeft, BarChart3, Camera, LucideIcon, PenTool, ShoppingCart, Wrench } from "lucide-react";
...
const CartSummaryStep4 = ({ selectedServices, onModifyConfiguration }: CartSummaryStep4Props) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative rounded-xl p-6 bg-apty-bg-elevated/90 backdrop-blur-sm border border-color-default shadow-xl">
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold bg-gradient-to-r from-brand-tertiary via-brand-accent to-brand-accent bg-clip-text text-transparent">
              Riepilogo Ordine
            </h3>
            {selectedServices.length > 0 && (
              <div className="px-3 py-1 bg-gradient-to-r from-brand-tertiary/20 to-brand-accent/20 rounded-full text-sm text-brand-tertiary font-medium">
            ...
            {selectedServices.length === 0 ? (
              <div className="bg-color-subtle/80 rounded-lg p-8 border border-color-default">
                <div className="flex flex-col items-center justify-center text-color-disabled">
                  ...
                  <p className="text-xs text-color-disabled mt-1">Seleziona i servizi che desideri aggiungere</p>
                </div>
              </div>
            ) : (
              servicesWithCategory.map((service, index) => (
                ...
                >
                  {service.categoryIcon && (
                    <motion.div
                      className="p-1.5 rounded-md bg-gradient-to-br from-brand-tertiary/10 to-brand-accent/10 text-brand-tertiary"
                      style={{
          ...
          <motion.button
            className="w-full mt-4 px-4 py-2 bg-apty-bg-elevated border-2 border-brand-tertiary text-brand-tertiary rounded-lg font-medium text-sm hover:bg-brand-tertiary/10 transition-all duration-300 shadow-md hover:shadow-lg flex items-center justify-center gap-2"
