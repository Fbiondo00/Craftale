...
import { ArrowRight, Check, Edit3, Lightbulb, RotateCcw, Star, Target, TrendingUp } from "lucide-react";
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
        ...
        <h2 className="text-3xl font-bold text-color-primary mb-2">Ecco la Soluzione Perfetta per Te!</h2>
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-color-primary">Compatibilità: {confidence}%</h3>
              <p className="text-color-tertiary">Questa soluzione è perfetta per le tue esigenze</p>
            </div>
          </div>

          <div className="text-right">
            <div className="text-3xl font-bold text-color-state-success-strong">{confidence}%</div>
            <div className="text-sm text-color-muted">Match Score</div>
          </div>
        </div>
      </motion.div>
      ...
      >
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="bg-white/20 px-3 py-1 rounded-full text-sm font-medium">Raccomandato per te</span>
              </div>
              <h3 className="text-2xl font-bold mb-1">
                {recommendedTier.name} - {recommendedPackage.name}
              </h3>
              <p className="text-white/90 text-lg">{recommendedTier.tagline}</p>
            </div>

            <div className="text-right">
              <div className="text-3xl font-bold">€{recommendedPackage.price.toLocaleString("it-IT")}</div>
              <div className="text-white/80">una tantum</div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            {/* Core Features */}
            <div>
              <h4 className="text-lg font-semibold text-color-primary mb-4">Cosa Include:</h4>
            ...
            {/* Package Stats */}
            <div>
              <h4 className="text-lg font-semibold text-color-primary mb-4">Dettagli del Progetto:</h4>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-color-subtle rounded-lg p-3">
                  <div className="text-2xl font-bold text-color-primary">{recommendedPackage.pages}</div>
                  <div className="text-sm text-color-tertiary">Pagine</div>
                </div>
                <div className="bg-color-subtle rounded-lg p-3">
                  <div className="text-2xl font-bold text-color-primary">{recommendedPackage.revisions}</div>
                  <div className="text-sm text-color-tertiary">Revisioni</div>
                </div>
                <div className="bg-color-subtle rounded-lg p-3">
                  <div className="text-sm font-medium text-color-primary">⏰ {recommendedPackage.timeline}</div>
                  <div className="text-xs text-color-tertiary">Consegna</div>
                </div>
                <div className="bg-color-subtle rounded-lg p-3">
                  <div className="text-sm font-medium text-color-primary">🆘 {recommendedPackage.support}</div>
          ...
          <div className="bg-color-state-info-bg rounded-xl p-4 mb-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-color-primary mb-2">Perché è perfetto per te:</h4>
      ...
      {optionalServices.length > 0 && (
        ...
        >
          <h3 className="text-xl font-semibold text-color-primary mb-4">Servizi Aggiuntivi Consigliati</h3>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {service.popular && (
                        <span className="bg-orange-100 text-orange-800 text-xs px-2 py-1 rounded-full">Popolare</span>
                      )}
                    </div>
                    <p className="text-sm text-color-tertiary mb-2">{service.description}</p>
                  </div>

                  <div className="text-right ml-4">
                    <div className="font-bold text-color-primary">€{service.price}</div>
                    {service.unit && (
      ...
      >
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div>
            ...
            <div className="text-2xl font-bold">€{totalEstimate.base.toLocaleString("it-IT")}</div>
          ...
          {totalEstimate.monthly && (
            <div>
              ...
              <div className="text-2xl font-bold text-blue-400">€{totalEstimate.monthly.toLocaleString("it-IT")}</div>
      ...
      {alternatives.length > 0 && (
        <motion.div
          ...
          className="bg-white rounded-2xl shadow-lg border border-color-default p-6"
        >
          <h3 className="text-xl font-semibold text-color-primary mb-4">Alternative da Considerare</h3>
              ...
              <div key={`${alt.tier.name}-${alt.package.name}`} className="border border-color-default rounded-xl p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-color-tertiary mb-2">{alt.reason}</p>
                    <div className="text-sm text-color-muted">{alt.package.description}</div>
                  </div>
