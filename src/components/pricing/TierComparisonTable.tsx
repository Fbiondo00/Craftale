...
import { TierFeatureValue, tierComparisonData } from "./tierComparisonData";
    ...
    if (tierValue.shortDescription || tierValue.longDescription) {
      ...
      return (
        <Tooltip content={tooltipContent}>
          <span className="border-b-2 border-dotted border-gray-300 hover:border-gray-500">{content}</span>
        </Tooltip>
      );
    }
...
const TableHeader: React.FC<TableHeaderProps> = ({ onTierSelect, isMobile = false }) => {
  if (isMobile) {
    return (
      <thead
        className='bg-white bg-opacity-100 sticky top-20 z-40 before:absolute before:inset-x-0 before:-top-20 before:h-20 before:bg-white before:content-[""]'
  ...
  return (
    <thead
      className='bg-white bg-opacity-100 sticky top-24 z-40 before:absolute before:inset-x-0 before:-top-24 before:h-24 before:bg-white before:content-[""]'
      style={{ transform: "translateZ(0)", willChange: "transform" }}
    >
      <tr className="bg-white bg-opacity-100">
        ...
        <th className="px-6 py-6 text-center bg-purple-50 bg-opacity-100 border-l-2 border-r-2 border-purple-200 relative">
          ...
          <AptySecondaryButton onClick={() => onTierSelect("essential")} className="min-w-[140px]" size="sm">
            Inizia con Starter
          </AptySecondaryButton>
        </th>
        <th className="px-6 py-6 text-center bg-white bg-opacity-100">
          <div className="text-xl font-semibold text-gray-900 mb-3">Pro</div>
          <AptyPrimaryButton onClick={() => onTierSelect("interactive")} className="min-w-[140px]" size="sm">
            Inizia con Pro
          </AptyPrimaryButton>
        </th>
        <th className="px-6 py-6 text-center bg-white bg-opacity-100">
          <div className="text-xl font-semibold text-gray-900 mb-3">Ecommerce</div>
          <AptySecondaryButton onClick={() => onTierSelect("ecommerce")} className="min-w-[140px]" size="sm">
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-28">
          ...
          <p className="text-lg text-gray-600 max-w-3xl mx-auto leading-relaxed">
            Confronta le caratteristiche dei nostri tre livelli di servizio e trova la soluzione ideale per le tue
            esigenze digitali.
          </p>
        </div>
                  ...
                  <React.Fragment key={categoryIndex}>
                    {isMobile ? (
                      <>
                        ...
                        <tr
                          ref={el => {
                            mobileCategoryRefs.current[categoryIndex] = el;
                          }}
                          className={categoryIndex === activeStickyIndex ? "sticky top-32" : ""}
                          style={
                            categoryIndex === activeStickyIndex
                              ? {
                                  zIndex: 30,
                        ...
                        >
                          <td
                            colSpan={3}
                            className="px-4 py-3 font-semibold text-gray-800 bg-gray-50 bg-opacity-100 text-center"
                          >
                            {category.category}
                          </td>
                        </tr>
                      ...
                      >
                        <td className="px-6 py-3 font-semibold text-gray-800 bg-gray-50 bg-opacity-100">
                          {category.category}
                        </td>
                      ...
                      return (
                        <tr key={featureIndex} className="group hover:bg-purple-50">
                          <td className="px-6 py-4 bg-white group-hover:bg-purple-50 border-b border-gray-100">
                            <FeatureName name={feature.name} longDescription={feature.longDescription} />
                          </td>
                          <td
