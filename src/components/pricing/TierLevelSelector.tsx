...
import { Check, X, Info, ChevronDown, Loader2 } from 'lucide-react';
import type { TierWithLevels } from '@/types/database-extended'; // removed , PricingLevel
...
const TierLevelSelector: React.FC<TierLevelSelectorProps> = ({
  ...
  const sortedLevels = [...selectedTier.levels].sort((a, b) =>
          ...
          },
          {
            ...
            levels: {
              ...
              C: {
                value: 'Professionale',
                longDescription:
                  'Avanzata + Analisi keyword, contenuti ottimizzati, link building interno',
              },
            },
          },
        ],
      ...
      <div className='container mx-auto px-4'>
                ...
                {featureComparison.map((category, categoryIndex) => (
                  <React.Fragment key={categoryIndex}>
                    ...
                    {category.features.map((feature, featureIndex) => {
                      const isLastFeature =
                        categoryIndex === featureComparison.length - 1 &&
                                ...
                                className={`px-6 py-4 text-center border-b border-apty-border-subtle ${
                                  isStandard
                                    ? `bg-apty-tertiary/10 border-l-2 border-r-2 border-apty-tertiary/30 ${
                                        isLastFeature
                                          ? 'border-b-2 border-b-apty-tertiary/30 rounded-b-xl'
                                          : ''
                                      }`
        ...
        <div className='mt-12 text-center'>
          <p className='text-apty-text-secondary mb-6'>
            Non sei sicuro di quale livello scegliere? Il nostro team ti aiuterà a trovare la
            soluzione perfetta
          </p>
          <button
