      ...
      <div className="relative rounded-xl overflow-hidden">
        ...
        <motion.div
          className={cn(
            ...
            selectedCount > 0 && "border-brand-tertiary/40 shadow-lg",
          )}
          style={{
            background:
              selectedCount > 0
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <motion.div
                className="p-2 rounded-lg text-brand-tertiary bg-gradient-to-br from-brand-tertiary/10 to-brand-accent/10 dark:from-brand-tertiary/20 dark:to-brand-accent/20"
                style={{
              ...
              <div>
                <h3 className="font-bold text-color-primary dark:text-color-text-inverse">{category.name}</h3>
                {selectedCount > 0 && (
                  <p className="text-xs text-color-muted dark:text-color-text-inverse/60">
                    {selectedCount} di {totalCount} serviz{selectedCount !== 1 ? "i" : "o"} selezionat
                    {selectedCount !== 1 ? "i" : "o"}
                  </p>
                )}
              </div>
            ...
            <div className="flex items-center gap-2">
              {selectedCount > 0 && (
                <motion.div
                  className="px-2 py-1 bg-gradient-to-r from-brand-tertiary/20 to-brand-accent/20 rounded-full text-xs text-brand-tertiary font-medium"
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
