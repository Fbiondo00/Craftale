  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed z-50 top-1/2 left-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl bg-color-elevated shadow-xl border border-color-default p-6 focus:outline-none">
          <Dialog.Title className="text-lg font-semibold text-color-primary mb-1">Torna indietro?</Dialog.Title>
          <Dialog.Description className="text-sm text-color-secondary mb-4 leading-relaxed">
            Stai lasciando il {currentStepLabel}. Vuoi salvare i progressi prima di tornare allo {targetStepLabel}? Puoi
            anche scegliere di non salvare e continuare.
          </Dialog.Description>
          <div className="flex flex-col gap-2">
            ...
            <button
              ...
              onClick={onDiscard}
              disabled={saving}
              className="w-full inline-flex items-center justify-center rounded-md border border-color-strong/40 bg-color-elevated text-color-primary text-sm font-medium py-2.5 hover:bg-color-subtle disabled:opacity-60 transition"
            >
              Non salvare
            </button>
            <button
