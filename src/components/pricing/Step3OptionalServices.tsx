...
import { motion } from 'framer-motion'; //removed animate
import {
  ChevronDown,
  Check,
  Sparkles,
    ...
    return (
      <motion.button
        ...
        className={cn(
          ...
          isOpen
            ? 'border-brand-secondary/40 bg-brand-secondary/10 shadow-md'
            : 'border-color-default hover:border-brand-secondary/30 hover:bg-color-subtle'
        )}
      ...
      >
          ...
          <div
            className={cn(
              ...
              isOpen
                ...
                : 'bg-color-muted text-color-tertiary group-hover:bg-brand-secondary/20'
            )}
          >
        ...
        <div className='flex items-center space-x-2'>
          {selectedCount > 0 && (
            <div className='bg-brand-secondary text-white text-xs font-bold px-2 py-1 rounded-full'>
              {selectedCount}
            </div>
          )}
          <ChevronDown
            className={cn(
              'w-5 h-5 text-color-disabled transition-transform duration-200',
              isOpen && 'rotate-180'
            )}
          />
        </div>
...
const ServiceCard: React.FC<ServiceCardProps> = memo(
  ({ service, isSelected, isCompatible, onToggle }) => {
      ...
      <motion.div
        className={cn(
          ...
          isSelected && isCompatible
            ? 'border-brand-secondary/90 bg-brand-secondary/10 shadow-lg'
            : isCompatible
          ...
          <div className='flex items-center space-x-3'>
            ...
            <div
              className={cn(
                ...
                isSelected && isCompatible ? 'border-brand-secondary/90 bg-brand-secondary/90' : 'border-color-strong'
              )}
            >
        ...
        {oneTimeTotal > 0 && (
          <div className='flex justify-between items-center'>
            ...
            <span className='text-lg font-bold text-color-primary'>
              €{oneTimeTotal.toLocaleString('it-IT')}
            </span>
          </div>
        )}
        ...
        {monthlyTotal > 0 && (
          <div className='flex justify-between items-center'>
            ...
            <span className='text-lg font-bold text-brand-secondary'>
              €{monthlyTotal.toLocaleString('it-IT')}/mese
            </span>
          </div>
        )}
      </div>
...
const Step3OptionalServices: React.FC<Step3OptionalServicesProps> = ({
  ...
  const getSelectedCountForCategory = (categoryId: string): number => {
    ...
    return selectedServices.filter((service) => category.services.some((s) => s.id === service.id))
      .length;
  };
    ...
    <div className={cn('space-y-8', className)}>
      <div className='text-center'>
        ...
        <p className='text-lg text-color-tertiary max-w-3xl mx-auto'>
          Personalizza la tua presenza digitale con servizi professionali aggiuntivi. Seleziona solo
