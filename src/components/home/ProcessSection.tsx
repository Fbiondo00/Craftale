...
const web44Stages: ProcessStage[] = [
  {
    ...
    description:
      "Analizziamo a fondo i tuoi obiettivi e individuiamo opportunità per posizionarti davanti alla concorrenza.",
    status: "completed",
    duration: "2-3 weeks",
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        >
          ...
          <p className="text-lg text-apty-text-secondary max-w-4xl mx-auto leading-relaxed">
            Non costruiamo solo siti web: progettiamo esperienze digitali che generano risultati. Il nostro processo
            collaudato trasforma la tua visione in una realtà ad alto tasso di conversione, senza sorprese e con il
            massimo impatto.
          </p>
        </motion.div>
            ...
            {web44Stages.map((stage, index) => {
              ...
              return (
                ...
                >
                    ...
                    <div className={`${isEven ? "md:pr-12" : "md:pl-12"}`}>
                      ...
                      <div className={`flex items-center gap-3 mb-3 ${isEven ? "md:flex-row-reverse" : ""}`}>
                        ...
                        <div className={`${isEven ? "md:text-right" : ""}`}>
                          ...
                          <p className="text-sm text-apty-tertiary font-medium">{stage.subtitle}</p>
                        </div>
                      </div>

                      {/* Description */}
                      <p className="text-apty-text-secondary mb-4 leading-relaxed">{stage.description}</p>

                      {/* Duration Tag */}
                      <div
                        className={`inline-flex items-center gap-2 px-3 py-1.5 bg-apty-primary/10 rounded-full ${
                          isEven ? "md:flex-row-reverse" : ""
                        }`}
                      >
                        <Clock className="w-3.5 h-3.5 text-apty-primary" />
                        <span className="text-xs font-medium text-apty-primary">{stage.duration}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <motion.div
                    className={`relative z-10 md:mt-0 ${stage.id === "4" || stage.id === "6" ? "mt-6" : "mt-4"}`}
                    ...
                    <div
                      ...
                      ${
                        stage.id === "1"
                          ...
                          : stage.id === "2"
                            ? "bg-apty-gradient-secondary"
                            : stage.id === "3"
                              ? "bg-apty-gradient-vibrant"
                              : stage.id === "4"
                                ? "bg-apty-gradient-vibrant"
                                : stage.id === "5"
                                  ? "bg-apty-gradient-secondary"
                  ...
                  <div className="flex-1 md:hidden">
                    <div className="bg-apty-bg-muted rounded-lg p-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-apty-tertiary font-medium">{stage.subtitle}</p>
                            </div>
                          </div>
                        </div>
                        {/* Duration tag */}
                      ...
                      {/* Description */}
                      <p className="text-sm text-apty-text-secondary mb-4 leading-relaxed">{stage.description}</p>
                  ...
                  <div className={`hidden md:flex flex-1 ${isEven ? "md:text-left" : "md:text-right"}`}>
                    ...
                    >
                      <div className="bg-apty-bg-muted rounded-lg p-4 group hover:bg-apty-bg-elevated apty-transition cursor-default">
                        <h4
                          className={`text-xs font-semibold text-apty-primary uppercase tracking-wider mb-3 flex items-center gap-2 ${
        ...
        >
          ...
          <p className="text-sm text-apty-text-secondary mt-3">Dal primo incontro ai risultati concreti</p>
        </motion.div>
      </div>
    </section>
  );
