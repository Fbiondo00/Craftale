...
export default function HeroSection({ onScrollToProcess }: HeroSectionProps) {
    ...
    const handleScroll = () => {
      ...
      animationFrameId.current = requestAnimationFrame(() => {
        ...
        if (inAnimationZone && !isAnimatingRef.current) {
          ...
          // Handle scroll down
          if (scrollDirection === "down" && mobileScrollProgress < 1.1) {
            // Allow overshoot
          ...
          // Handle scroll up
          else if (scrollDirection === "up" && mobileScrollProgress > -0.1) {
            // Allow negative for full start
            // Calculate velocity-based target - faster scroll = more distance
            const velocity = Math.abs(deltaY);
            const velocityMultiplier = Math.min(1.5, 1 + velocity * 0.01); // Cap at 1.5x
  ...
  return (
    <>
      ...
      <section
        ref={heroRef}
        className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20 bg-apty-bg-subtle"
      >
