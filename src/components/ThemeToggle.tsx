...
export const ThemeToggle = memo(() => {
  ...
  const transitionClass = useMemo(
    () => prefersReducedMotion ? '' : 'apty-transition',
  ...
  const iconTransform = useMemo(
    () => ({
      sun: isDarkMode 
        ? prefersReducedMotion ? 'opacity-0' : 'opacity-0 rotate-180 scale-0'
        : prefersReducedMotion ? 'opacity-100' : 'opacity-100 rotate-0 scale-100',
      moon: isDarkMode
        ? prefersReducedMotion ? 'opacity-100' : 'opacity-100 rotate-0 scale-100'
        : prefersReducedMotion ? 'opacity-0' : 'opacity-0 -rotate-180 scale-0',
    }),
