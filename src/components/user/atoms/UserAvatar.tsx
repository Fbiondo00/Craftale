    ...
    return (
      <div
        ...
        className={cn(
          ...
          onClick && 'cursor-pointer focus:outline-none focus:ring-2 focus:ring-brand-tertiary/90 focus:ring-offset-2 rounded-full',
          className
        )}
        onClick={onClick}
        {...props}
      >
        <Avatar className={cn(avatarSizes[size])}>
          ...
          >
            {user.avatar ? (
              <span className={cn('text-xs font-semibold', size === 'xl' ? 'text-lg' : 'text-xs')}>
                {initials}
              </span>
            ) : (
