...
export const ForgotPasswordForm = React.forwardRef<HTMLFormElement, ForgotPasswordFormProps>(
  ({ onSubmit, onBackToSignIn, className, ...props }, ref) => {
    ...
    return (
      ...
      >
        <CardHeader className='text-center pb-4'>
          <CardTitle className='text-2xl font-semibold font-apty-heading text-apty-text-primary'>Forgot Password?</CardTitle>
        ...
        <CardContent className='space-y-6'>
          <form ref={ref} onSubmit={handleSubmit} className='space-y-4' {...props}>
            ...
            {forgotPasswordErrors.general && (
              <ErrorMessage>{forgotPasswordErrors.general}</ErrorMessage>
            )}
