...
import { ForgotPasswordForm, SignInForm, SignUpForm } from "./organisms";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { cn } from "@/lib/utils";
import { AuthMode, ForgotPasswordFormData, SignInFormData, SignUpFormData } from "@/types/auth";
import { AnimatePresence, motion } from "framer-motion";
import { CheckCircle, Palette, Target, Zap } from "lucide-react";
...
export const AuthContainer = React.forwardRef<HTMLDivElement, AuthContainerProps>(
  (
    ...
    if (isSuccess) {
      return (
        <div className={cn("min-h-screen bg-apty-bg-base flex items-center justify-center p-4", className)}>
          <motion.div
    ...
    return (
      <div
        ref={ref}
        className={cn("min-h-screen bg-apty-bg-base flex items-center justify-center p-4", className)}
        {...props}
      >
        ...
        <div className="relative w-full max-w-6xl mx-auto grid lg:grid-cols-2 gap-8 items-center">
          ...
          >
            <div className="space-y-4">
              <a href="/" className="flex items-center space-x-3 group hover:scale-[1.02] apty-transition w-fit">
                <img src="/logo.png" alt="Craftale Logo" className="w-12 h-12" />
                <div className="flex flex-col">
                  <span className="text-2xl font-bold apty-gradient-text">Craftale</span>
              ...
              <h1 className="text-4xl lg:text-5xl font-bold text-apty-text-primary font-apty-heading leading-tight">
                ...
                <br />
                <span className="apty-gradient-text">realtà digitali</span>
              </h1>
              <p className="text-xl text-apty-text-secondary leading-relaxed">
                Sviluppiamo soluzioni web innovative per far crescere il tuo business e raggiungere nuovi traguardi nel
                mondo digitale.
              </p>
            </div>
          ...
          >
            ...
            >
              <img src="/logo.png" alt="Craftale Logo" className="w-10 h-10" />
              <div className="flex flex-col">
                <span className="text-xl font-bold apty-gradient-text">Craftale</span>
                <span className="text-xs text-apty-text-secondary font-medium">Website Builders</span>
              </div>
            </a>
            ...
            <AnimatePresence mode="wait">
              {mode === "signin" ? (
                ...
                <motion.div
                  ...
                  transition={{ duration: 0.3 }}
                >
                  <SignUpForm onSubmit={handleSignUpSubmit} onModeToggle={toggleMode} onSocialAuth={handleSocialAuth} />
                </motion.div>
              ) : (
                <motion.div
                  key="forgot-password"
                  ...
                  transition={{ duration: 0.3 }}
                >
                  <ForgotPasswordForm onSubmit={handleForgotPasswordSubmit} onBackToSignIn={handleBackToSignIn} />
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
